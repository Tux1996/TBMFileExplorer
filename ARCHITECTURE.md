...
- **Manual UI click-through beyond the initial connection.** The Add Server sheet, host-key confirmation dialog, and connecting to a real server have been clicked through by hand (against the user's actual server — this is what surfaced the login-timeout bug above). Browsing/file operations within a connected remote tab, remote drag-and-drop, and the "host key changed" re-confirmation dialog specifically have not been — this environment has no way to script further clicks into the native window (see `TESTING.md`), so that's on the user or a future session with hands on the app.
