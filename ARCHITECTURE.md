...
**What works, verified by a real (if fast/deterministic, using an in-memory `FileProvider` fake) test suite** (`AppTests/TransferManagerTests.swift`, 9 tests): enqueuing a batch and streaming a file to completion with matching bytes; every collision resolution (Replace/Skip/Keep Both/Resume) doing what it says, including Resume correctly picking up from the destination's existing size; "Apply to All" reusing the first decision for the rest of a batch without re-prompting; pausing a running transfer (verified it doesn't silently keep making progress) and resuming it to completion; cancelling a running transfer actually stopping it; retrying a failed job resetting its state and succeeding once the underlying problem is fixed.

**And verified by hand against a real server:** dragging a file from the user's actual SFTP connection into a local Documents folder actually transferred it — the first genuine end-to-end proof that cross-provider transfer works outside of tests, once bug 3 below was found and fixed.

**Three real bugs this project had to discover by testing, not by inspection** (recorded here so nobody re-derives them the hard way):
...
3. **A remote file's drag payload was invisible to every drop target — no cursor change, nothing.** `DragPayloadKind`'s custom UTI (carrying `(providerKey, path)` for a remote row, which has no real local file to fall back on) was declared purely at runtime via `UTType(exportedAs:conformingTo:)`. That initializer is meant for exactly this "no Info.plist entry available" case, but in practice the app logged `[Type Declaration Issues] Type "..." was expected to be declared and exported in the Info.plist ... but it was not found` on every launch — a warning that's easy to scroll past — and the drag session itself never recognized the type as droppable anywhere, so a Gibson→Mac drag looked like it wasn't registering at all. Fixed by properly declaring the UTI as an exported type in `Info.plist` (`project.yml`'s `UTExportedTypeDeclarations`) instead of relying on the dynamic-only initializer. Verified by hand: a real Gibson→Mac Documents drag now transfers the file. Lesson for later: a console warning about type declarations during a feature's first real test is never noise, even when it's not the thing you're actively debugging at that moment.
