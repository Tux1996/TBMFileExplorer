...
- **Manual UI click-through beyond connect/rename/delete.** The Add Server sheet, host-key confirmation dialog, connecting to a real server, renaming, and deleting a remote file have all been clicked through by hand against the user's actual server (this is what surfaced the login-timeout bug above, and then confirmed it fixed). Permissions, drag-and-drop for remote tabs, and the "host key changed" re-confirmation dialog specifically have not been — this environment has no way to script further clicks into the native window (see `TESTING.md`), so that's on the user or a future session with hands on the app.
